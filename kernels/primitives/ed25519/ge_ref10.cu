// ge_ref10.cu - Group operations implementation (ref10)
// Includes and precomp_data.h already included via ed25519_ref10.cuh
...
static void __host__ __device__ select_from_table(ge_precomp *t, const ge_precomp *base_table, int pos, signed char b) {
    ge_precomp minust;
    ...
    fe_1(t->yminusx);
    fe_0(t->xy2d);
    // base_table is a flat array: base[pos][j] = base_table[pos * 8 + j]
    const ge_precomp *row = base_table + pos * 8;
    cmov(t, &row[0], equal(babs, 1));
    cmov(t, &row[1], equal(babs, 2));
    cmov(t, &row[2], equal(babs, 3));
    cmov(t, &row[3], equal(babs, 4));
    cmov(t, &row[4], equal(babs, 5));
    cmov(t, &row[5], equal(babs, 6));
    cmov(t, &row[6], equal(babs, 7));
    cmov(t, &row[7], equal(babs, 8));
...
void __device__ __host__ ge_scalarmult_base_with_table(ge_p3 *h, const unsigned char *a, const ge_precomp *base_table) {
    signed char e[64];
    signed char carry;
    ge_p1p1 r;
    ge_p2 s;
    ...
    for (i = 1; i < 64; i += 2) {
        select_from_table(&t, base_table, i / 2, e[i]);
        ge_madd(&r, h, &t);
        ge_p1p1_to_p3(h, &r);
    }
...
// Original function for backward compatibility (uses static table)
